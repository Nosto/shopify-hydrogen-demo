import { GraphQLError, Kind, parse, specifiedDirectives, specifiedScalarTypes, } from "graphql";
...
export function isFederationVersion(version) {
    return version in federationSpecFactory;
}
export function createSpecSchema(version, imports) {
    if (!isFederationVersion(version)) {
        throw new GraphQLError(`Invalid version ${version} for the federation feature in @link directive on schema`, {
            extensions: {
...
function createTypeDefinitions(doc, prefix, imports) {
    ...
    return {
        directives: directives.filter((d) => !specifiedDirectives.some((sd) => sd.name === d.name.value) &&
...
export function isFederationLink(link) {
    return link.identity === "https://specs.apollo.dev/federation";
}
export function detectFederationVersion(typeDefs) {
        ...
        if (definition.kind === Kind.SCHEMA_EXTENSION ||
            ...
            if (links?.length) {
                const parsedLinks = links.map((l) => {
                    ...
                    return parseLink(url.value.value, importArg ? print(importArg.value) : "[]");
                });
