...
import { LazyErrors } from "./errors.js";
...
export class Walker {
    ...
    walkTrail(operationType, steps) {
        ...
        let state = new WalkTracker(new OperationPath(rootNode), this.mergedGraph
            .nodesOf(rootNode.typeName, false)
            .map((n) => new OperationPath(n)), new OverrideLabels());
    ...
    dfs() {
        ...
        for (const rootNode of rootNodes) {
            this._dfs(rootNode, new WalkTracker(new OperationPath(rootNode), this.mergedGraph
                .nodesOf(rootNode.typeName, false)
                .map((n) => new OperationPath(n)), overrideLabels), unreachable);
        }
        return unreachable;
    }
    ...
    bfs() {
        ...
        for (const name of ["Query", "Mutation", "Subscription"]) {
            ...
            queue.push(new WalkTracker(new OperationPath(rootNode), this.mergedGraph
                .nodesOf(rootNode.typeName, false)
                .map((n) => new OperationPath(n)), overrideLabels));
        }
