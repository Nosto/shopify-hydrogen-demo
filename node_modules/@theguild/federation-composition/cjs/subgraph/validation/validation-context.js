...
function createSimpleValidationContext(typeDefs, typeNodeInfo) {
    ...
    for (const definition of typeDefs.definitions) {
        if (definition.kind === graphql_1.Kind.DIRECTIVE_DEFINITION) {
            ...
            definition.name &&
            definition.name.kind === graphql_1.Kind.NAME) {
        ...
        getSchemaCoordinate(ancestors) {
            ...
            for (let i = 0; i < ancestors.length; i++) {
                ...
                if ("kind" in ancestor && ancestor.kind !== graphql_1.Kind.DOCUMENT) {
                    const name = ancestor.kind === graphql_1.Kind.SCHEMA_DEFINITION ||
                        ancestor.kind === graphql_1.Kind.SCHEMA_EXTENSION
                        ? "schema"
...
function createSubgraphValidationContext(subgraph, federation, typeNodeInfo, stateBuilder) {
    ...
    for (const def of subgraph.typeDefs.definitions) {
        ...
        typenameToKind.set(def.name.value, def.kind === graphql_1.Kind.OBJECT_TYPE_DEFINITION ||
            def.kind === graphql_1.Kind.OBJECT_TYPE_EXTENSION
            ? state_js_2.TypeKind.OBJECT
            : state_js_2.TypeKind.INTERFACE);
        ...
        if (def.kind === graphql_1.Kind.OBJECT_TYPE_DEFINITION &&
            found.kind === graphql_1.Kind.OBJECT_TYPE_EXTENSION) {
    ...
    return {
        ...
        getFieldsToMarkAsShareable() {
            ...
            for (const coordinate of markedAsKeyField) {
                ...
                if (typenameToKind.get(typeName) === state_js_2.TypeKind.OBJECT) {
        ...
        collectUnusedExternal() {
            ...
            return unused.filter((coordinate) => {
                ...
                if (typeDef.kind === state_js_2.TypeKind.OBJECT &&
                    ...
                    for (const interfaceName of typeDef.interfaces) {
                        ...
                        if (iDef.kind === state_js_2.TypeKind.INTERFACE &&
