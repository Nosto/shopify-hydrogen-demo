...
class Graph {
            ...
            for (const otherNodeIndex of sameTypeNameNodeIndexes) {
                ...
                for (const tailNode of this.nodesByTypeIndex[otherNodeIndex]) {
                    ...
                    for (const key of keys) {
                        if (key.resolvable) {
                            edgesToAdd.push(new edge_js_1.Edge(headNode, tailNode.typeState.kind === "object"
                                ? new moves_js_1.EntityMove(this.selectionResolver.resolve(headNode.typeName, key.fields))
                                : new moves_js_1.AbstractMove(this.selectionResolver.resolve(headNode.typeName, key.fields)), tailNode));
                        }
                    }
                }
            }
    ...
    addProvidedField(head, providedField, queue) {
        ...
        for (const index of indexes) {
            ...
            const newTail = this.duplicateNode(edge.tail, edge.move);
            const newEdge = new edge_js_1.Edge(edge.head, new moves_js_1.FieldMove(edge.move.typeName, edge.move.fieldName, edge.move.requires, edge.move.provides, null, true), newTail);
            this.replaceEdgeAt(edge.head.index, edge.tail.index, newEdge, index);
                    ...
                    for (const interfaceName of typeNode.typeState.interfaces) {
                        ...
                        for (const interfaceNode of interfaceNodes) {
                            ...
                            for (const key of keys) {
                                ...
                                edgesToAdd.push(new edge_js_1.Edge(head, new moves_js_1.AbstractMove(this.selectionResolver.resolve(interfaceName, key.fields)), interfaceNode));
                            }
                        }
                    }
                }
            ...
            for (const superFieldEdge of this.superFieldEdgesToApplyOverride) {
                ...
                for (const [_, fieldStateInGraph] of fieldState.byGraph) {
                    if (fieldStateInGraph.overrideLabel && fieldStateInGraph.override) {
                        ...
                        this.addEdge(new edge_js_1.Edge(superFieldEdge.head, new moves_js_1.FieldMove(typeName, fieldName, null, null, {
                            label: fieldStateInGraph.overrideLabel,
                            value: false,
                            fromGraphId: null,
                        }), superFieldEdge.tail));
    ...
    duplicateNode(originalNode, move) {
        ...
        if (move instanceof moves_js_1.FieldMove) {
    ...
    createEdgeForInterfaceTypeField(head, field) {
        ...
        return this.addEdge(new edge_js_1.Edge(head, new moves_js_1.FieldMove(head.typeName, field.name, requires
    ...
    createEdgeForObjectTypeField(head, field) {
        ...
        const outputTypeName = (0, state_js_1.stripTypeModifiers)(fieldType);
        const tail = this.createNodesAndEdgesForType(outputTypeName);
        if (!tail) {
            throw new Error(`Failed to create Node for ${outputTypeName} in subgraph ${this.id}`);
        }
        if (this.isSupergraph()) {
            const edge = new edge_js_1.Edge(head, new moves_js_1.FieldMove(head.typeName, field.name), tail);
    ...
    isMergedGraph() {
        return this.idSymbol === constants_js_1.MERGEDGRAPH_ID;
    }
}
exports.Graph = Graph;
