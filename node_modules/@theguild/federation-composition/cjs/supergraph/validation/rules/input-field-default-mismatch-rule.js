...
exports.InputFieldDefaultMismatchRule = InputFieldDefaultMismatchRule;
const graphql_1 = require("graphql");
function InputFieldDefaultMismatchRule(context) {
    return {
        InputObjectTypeField(inputObjectState, fieldState) {
            ...
            if (defaultValueToGraphs.size > 1) {
                ...
                context.reportError(new graphql_1.GraphQLError(`Input field "${inputObjectState.name}.${fieldState.name}" has incompatible default values across subgraphs: it has ${first} but ${second}${rest.length ? ` and ${rest.join(" and ")}` : ""}`, {
                    extensions: {
