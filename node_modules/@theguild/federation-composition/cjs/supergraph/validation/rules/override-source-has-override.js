...
exports.OverrideSourceHasOverrideRule = OverrideSourceHasOverrideRule;
const graphql_1 = require("graphql");
function OverrideSourceHasOverrideRule(context) {
    return {
            ...
            for (let i = 0; i < graphsWithOverride.length; i++) {
                ...
                while (overrideValue) {
                    if (visitedGraphs.has(overrideValue)) {
                        context.reportError(new graphql_1.GraphQLError(`Field "${objectTypeState.name}.${fieldState.name}" on subgraph "${context.graphIdToName(graph)}" is also marked with directive @override in subgraph "${context.graphIdToName(overrideValue)}". Only one @override directive is allowed per field.`, {
                            extensions: {
